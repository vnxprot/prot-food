...
export default function Home() {
  ...
  const [locationPermission, setLocationPermission] = useState<"idle" | "requesting" | "granted" | "denied" | "unavailable" | "timeout">("idle");
  ...
  const [showAllManagedCategories, setShowAllManagedCategories] = useState(false);
        ...
        <section className="min-w-0 flex-1 px-4 pb-28 pt-6 sm:px-6 md:px-10 md:pb-10">
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0 flex-1 flex items-center gap-1.5">
                      ...
                      <button
                        type="button"
                        onClick={getLocation}
                        className={`rounded-lg px-2.5 py-1.5 text-xs font-extrabold transition ${position ? "bg-[#402c1e]/8 p-1.5 text-[#6b5644] hover:bg-[#402c1e]/15 dark:bg-white/10 dark:text-[#cbb4a0]" : "bg-[#a35e2d] text-white shadow-sm hover:bg-[#7f421e]"}`}
          ...
          {tab === "settings" && (
            <div className="space-y-4">
              ...
              {isAdmin && <section className="glass rounded-[20px] p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-extrabold">Quản lý nhóm món ăn</p><p className="mt-1 text-xs text-[#8a7360]">Đổi tên hoặc xoá nhóm sẽ áp dụng cho toàn bộ quán thuộc nhóm đó.</p></div>{categories.length > 8 && <button type="button" onClick={() => setShowAllManagedCategories((value) => !value)} className="shrink-0 rounded-xl bg-[#402c1e]/7 px-2.5 py-2 text-xs font-bold text-[#402c1e] dark:bg-white/10 dark:text-[#f7eadc]">{showAllManagedCategories ? "Thu gọn" : `Tất cả (${categories.length})`}</button>}</div><div className="mt-3 space-y-2">{categories.length ? (showAllManagedCategories ? categories : categories.slice(0, 8)).map((value) => <div key={value} className="flex items-center justify-between gap-3 rounded-xl border border-[#402c1e]/10 bg-white/45 px-3 py-2.5 dark:bg-black/10"><span className="min-w-0 truncate text-sm font-bold">{value}</span><div className="flex shrink-0 gap-1"><button type="button" aria-label={`Đổi tên nhóm ${value}`} onClick={() => { const next = window.prompt(`Tên mới cho nhóm “${value}”:`, value)?.trim(); if (next && next !== value) void updateCategoryName(value, next); }} className="rounded-lg p-2 text-[#a35e2d] hover:bg-[#a35e2d]/10"><Pencil size={15} /></button><button type="button" aria-label={`Xoá nhóm ${value}`} onClick={() => void updateCategoryName(value, null)} className="rounded-lg p-2 text-red-700 hover:bg-red-700/10"><Trash2 size={15} /></button></div></div>) : <p className="py-2 text-xs text-[#8a7360]">Chưa có nhóm món nào.</p>}</div></section>}
